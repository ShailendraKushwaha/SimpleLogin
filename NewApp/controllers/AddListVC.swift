...
class AddListVC: UIViewController {
    ...
    lazy var nameField:UITextField = {
        let textField = UITextField()
        textField.translatesAutoresizingMaskIntoConstraints = false
        textField.placeholder = "Name of product"
        textField.borderStyle = .none
        textField.autocapitalizationType = .none
        textField.delegate =  self
        return textField
    ...
    lazy var phoneField:UITextField = {
        let textField = UITextField()
        textField.translatesAutoresizingMaskIntoConstraints = false
        textField.placeholder = "Left Amount"
    ...
    @objc public func createAccountTapped(sender:Any) {
        ...
        item.setValue(emailTextField.text!, forKey: "icon")
        item.setValue(Int(phoneField.text!), forKey: "left")
        item.setValue(Int(passwordField.text!), forKey: "total")
        print("Storing Data..")
                do {
                    try context.save()
                    self.navigationController?.popViewController(animated: true)
